// =====================================================================
//  ui.js — Utilidades de interfaz (toasts, formato, badges, modales)
// =====================================================================
const UI = {
  // ---- Toasts ----
  toast(msg, tipo = 'info', ms = 3600) {
    let wrap = document.querySelector('.toast-wrap');
    if (!wrap) { wrap = document.createElement('div'); wrap.className = 'toast-wrap'; document.body.appendChild(wrap); }
    const icon = { ok: ICON.check, err: ICON.alert, warn: ICON.alert, info: ICON.info }[tipo] || ICON.info;
    const el = document.createElement('div');
    el.className = 'toast ' + (tipo === 'info' ? '' : tipo);
    el.innerHTML = `${icon}<span>${UI.esc(msg)}</span>`;
    wrap.appendChild(el);
  ...
  date(d) {
    ...
    return x.toLocaleDateString('es-EC', { year: 'numeric', month: 'short', day: 'numeric' });
  },
  datetime(d) {
    if (!d) return '—';
    const x = new Date(d);
    if (Number.isNaN(x.getTime())) return '—';
    return x.toLocaleString('es-EC', { dateStyle: 'medium', timeStyle: 'short' });
  },

  // ---- Estados ----
  estadoLabel(e) { return ({ operativo: 'Operativo', mantenimiento: 'En mantenimiento', baja: 'Dado de baja' })[e] || e; },
  estadoClase(e) { return ({ operativo: 'op', mantenimiento: 'mant', baja: 'baja' })[e] || 'neutral'; },
  badge(e) {
    const c = UI.estadoClase(e);
    return `<span class="badge ${c}"><span class="dot"></span>${UI.estadoLabel(e)}</span>`;
  },

  // ---- Modales ----
  openModal(id) { const m = document.getElementById(id); if (m) m.classList.add('show'); },
  closeModal(id) { const m = document.getElementById(id); if (m) m.classList.remove('show'); },

  // Confirmación con modal propio → devuelve Promise<boolean>
  confirm(mensaje, { titulo = 'Confirmar', ok = 'Aceptar', danger = false } = {}) {
    return new Promise((resolve) => {
      const bg = document.createElement('div');
      bg.className = 'modal-bg show';
      bg.innerHTML = `
        <div class="modal" style="max-width:440px">
          <div class="modal-head"><h3>${UI.esc(titulo)}</h3></div>
          <div class="modal-body"><p style="margin:0;color:var(--ink-2)">${UI.esc(mensaje)}</p></div>
          <div class="modal-foot">
            <button class="btn btn-ghost" data-no>Cancelar</button>
            <button class="btn ${danger ? 'btn-danger' : 'btn-primary'}" data-yes>${UI.esc(ok)}</button>
          </div>
        </div>`;
      document.body.appendChild(bg);
      const fin = (v) => { bg.remove(); resolve(v); };
      bg.querySelector('[data-no]').onclick = () => fin(false);
      bg.querySelector('[data-yes]').onclick = () => fin(true);
      bg.onclick = (e) => { if (e.target === bg) fin(false); };
    });
  },

  // ---- Skeletons ----
  skelRows(cols, rows = 6) {
    let out = '';
    for (let r = 0; r < rows; r++) {
      out += '<tr>';
      for (let c = 0; c < cols; c++) out += `<td><div class="skel" style="height:14px;width:${50 + Math.random() * 45}%"></div></td>`;
      out += '</tr>';
    }
    return out;
  }
...
// =====================================================================
//  ICON — set de íconos SVG en línea (sin dependencias externas)
// =====================================================================
const ICON = {
  ...
  report: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M16 13H8"/><path d="M16 17H8"/><path d="M10 9H8"/></svg>',
  ...
  image: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/></svg>',
  ...
  pdf: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M10 12v6"/><path d="M14 12v6"/><path d="M10 15h4"/></svg>',
  excel: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="m9 13 6 6"/><path d="m15 13-6 6"/></svg>',
